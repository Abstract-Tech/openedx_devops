...
.. code-block:: shell
  ...
  kubectl get secret bastion-ssh-key -n abzt-openedx-eu-prod  -o json | jq  '.data | map_values(@base64d)' |   jq -r 'keys[] as $k | "export \($k|ascii_upcase)=\(.[$k])"'

  # 2.) save the private key to a file on your local dev machine and set permissions as required by AWS
  vim ~/.ssh/bastion.service.openedx-devops.abzt.de.pem
  sudo chmod 400 ~/.ssh/bastion.service.openedx-devops.abzt.de.pem

  # 3.) connect to the bastion server via ssh
  ssh bastion.service.openedx-devops.abzt.de -i ~/.ssh/bastion.service.openedx-devops.abzt.de.pem
...
.. code-block:: shell
  ...
  mysql -h mysql.service.openedx-devops.abzt.de -u root -p

  mongo --port 27017 --host mongo.service.openedx-devops.abzt.de

  redis-cli -h redis.service.openedx-devops.abzt.de -p 6379
